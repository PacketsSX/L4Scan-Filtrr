...
if len(sys.argv) < 2:
    print '[!] MUTLIAMP FILTER BY PACKETS.SX TEAM'
    print '[!] Usage: filter.py UnfilteredInput FilteredOutput ProtocolHere MinResponseSize ListFormatHere'
    print '[!] Protocols: chargen, ntp, quake, ssdp, ldap, dns, snmp, mdns, tftp, portmap, netbios'
    print '[!] List format variables: [space], [ip], [bytes]'
    print '[!] Example Useage: python2 filter.py unfiltered.txt filtered.txt ntp 440 [ip][space][bytes]'
    print '[!] This uses the script to filter through a fresh ntp reflector list'
    print '[!] for reflectors that respond with a size minimum of 440 into a new list formatted with IPHere ResponseSizeHere'
    sys.exit()
